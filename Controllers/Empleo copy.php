	...
	public function getBanners()
	{
		if ($_SESSION['permisosMod']['r']) {
			...
			for ($i = 0; $i < count($arrData); $i++) {
				...
				$btnDelete = '';
				if ($_SESSION['permisosMod']['r']) {
					$btnView = '<button class="btn btn-info btn-sm btnViewUsuario" onClick="fntViewBanner(' . $arrData[$i]['IdBaner'] . ')" title="Ver Empleo"><i class="far fa-eye"></i></button>';
				}
				if ($_SESSION['permisosMod']['u']) {
					if (($_SESSION['idUser'] == 1 and $_SESSION['userData']['idrol'] == 1) ||
						($_SESSION['userData']['idrol'] == 1 and $arrData[$i]['idrol'] != 1)
					) {
						$btnEdit = '<button class="btn btn-primary  btn-sm fntEditBanner" onClick="fntEditBanner(this,' . $arrData[$i]['IdBaner'] . ')" title="Editar Empleo"><i class="fas fa-pencil-alt"></i></button>';
					} else {
				...
				if ($_SESSION['permisosMod']['d']) {
					if (($_SESSION['idUser'] == 1 and $_SESSION['userData']['idrol'] == 1) ||
						...
						$btnDelete = '<button class="btn btn-danger btn-sm fntDelBanner" onClick="fntDelBanner(' . $arrData[$i]['IdBaner'] . ')" title="Eliminar Empleo"><i class="far fa-trash-alt"></i></button>';
					} else {
