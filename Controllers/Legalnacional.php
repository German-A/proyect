...
class legalnacional extends Controllers
{
	...
	public function legalnacional()
	{
		if (empty($_SESSION['permisosMod']['r'])) {
			header("Location:" . base_url() . '/dashboard');
		}
		$data['page_tag'] = "legalnacional";
		$data['page_title'] = "legalnacional <small>Unidad de Seguimiento del Egresado</small>";
		$data['page_name'] = "legalnacional";
		$data['page_functions_js'] = "functions_legalnacional.js";
		$this->views->getView($this, "legalnacional", $data);
	}
	...
	public function get()
	{
		if ($_SESSION['permisosMod']['r']) {
			...
			for ($i = 0; $i < count($arrData); $i++) {
				...
				$btnDelete = '';
				if ($_SESSION['permisosMod']['r']) {
					$btnView = '<button class="btn btn-info btn-sm fntView" onClick="fntView(' . $arrData[$i]['IdNacional'] . ')" title="Ver Legal"><i class="far fa-eye"></i></button>';
				}
				if ($_SESSION['permisosMod']['u']) {
					if (( $_SESSION['userData']['idrol'] == 1) ||( $_SESSION['userData']['idrol'] == 2)) {
						$btnEdit = '<button class="btn btn-primary  btn-sm fntEdit" onClick="fntEdit(this,' . $arrData[$i]['IdNacional'] . ')" title="Editar Banner"><i class="fas fa-pencil-alt"></i></button>';
					} else {
				...
				if ($_SESSION['permisosMod']['d']) {
					if (( $_SESSION['userData']['idrol'] == 1) ||( $_SESSION['userData']['idrol'] == 2)) {
						$btnDelete = '<button class="btn btn-danger btn-sm fntDelete" onClick="fntDelete(' . $arrData[$i]['IdNacional'] . ')" title="Eliminar Legal"><i class="far fa-trash-alt"></i></button>';
					} else {
