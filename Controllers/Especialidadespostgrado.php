...
class especialidadespostgrado extends Controllers
{
	...
	//pagina Banner
	public function especialidadespostgrado($i)
	{
		if (empty($_SESSION['permisosMod']['r'])) {
			header("Location:" . base_url() . '/dashboard');
		}
		$data['page_tag'] = "Postgrado";
		$data['page_title'] = "Detalle <small>Unidad de Seguimiento del Egresado</small>";
		$data['page_name'] = "USE-banner";
		$data['idespecialidad'] = $i;
		$data['page_functions_js'] = "functions_postgrado.js";
		$this->views->getView($this, "especialidadespostgrado", $data);
	}

		//listado de los banners
		public function detallegetpostgrado($id)
		{
			
			if ($_SESSION['permisosMod']['r']) {
				$arrData = $this->model->listadetallepostgrado($id);
				for ($i = 0; $i < count($arrData); $i++) {
					...
					if ($_SESSION['permisosMod']['r']) {
						$btnView = '<button class="btn btn-info btn-sm fntView" onClick="fntView(' . $arrData[$i]['iddetallepostgrado'] . ')" title="Ver Banner"><i class="far fa-eye"></i></button>';
					}
					if ($_SESSION['permisosMod']['u']) {
						if (($_SESSION['userData']['idrol'] == 1) || ($_SESSION['userData']['idrol'] == 2)) {
							$btnEdit = '<button class="btn btn-primary  btn-sm fntEdit" onClick="fntEdit(this,' . $arrData[$i]['iddetallepostgrado'] . ')" title="Editar Banner"><i class="fas fa-pencil-alt"></i></button>';
						} else {
					...
					if ($_SESSION['permisosMod']['d']) {
						if (($_SESSION['userData']['idrol'] == 1) || ($_SESSION['userData']['idrol'] == 2)) {
							$btnDelete = '<button class="btn btn-danger btn-sm fntDelete" onClick="fntDelete(' . $arrData[$i]['iddetallepostgrado'] . ')" title="Eliminar Banner"><i class="far fa-trash-alt"></i></button>';
						} else {
