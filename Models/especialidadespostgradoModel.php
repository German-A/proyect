<?php 
	...
	class EspecialidadespostgradoModel extends Mysql
	{
		...
		public function listadetallepostgrado($id)
		{
			$sql = "SELECT * 
			from postgrado p
			inner join escuela e on e.idEscuela=p.escuelaid
			inner join detallepostgrado dt on p.idpostgrado=dt.postgradoid
			where dt.postgradoid =$id and p.status!=0";
