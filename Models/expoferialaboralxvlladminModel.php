...
class ExpoferialaboralxvadminModel extends Mysql
{
	...
	{
		$sql = "SELECT * FROM expoxvllgaleria where status>0";
	...
	{
		$sql = "SELECT * FROM expoxvllponencias where status > 0";
		$request = $this->select_all($sql);
		return $request;
	}
	...
	{
		$sql = "SELECT archivo FROM expoxvllponencias where archivo = '$filename'";
		$request = $this->select_all($sql);
		return $request;
	}
