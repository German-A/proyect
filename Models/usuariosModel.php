<?php 
	...
	public function obte()
		...
		public function selectUsuarios()
		{
			...
			$sql = "SELECT p.idpersona,p.nombres,p.apellidop,p.telefono,p.email_user,p.status,r.idrol,r.nombrerol 
					FROM usuario p 
					INNER JOIN rol r
					ON p.rolid = r.idrol
		...
		public function selectUsuario(int $idpersona){
			...
			$sql = "SELECT p.idpersona,p.nombres,p.apellidop,p.telefono,p.email_user,r.idrol,r.nombrerol,p.status, DATE_FORMAT(p.datecreated, '%d-%m-%Y') as fechaRegistro 
					FROM usuario p
					INNER JOIN rol r
					ON p.rolid = r.idrol
			...
			if($request['idrol']==4){
				...
				$sql = "SELECT p.idpersona,p.nombres,p.apellidop,p.telefono,p.email_user,r.idrol,r.nombrerol,
		...
		public function updatePerfil(int $idUsuario,  string $nombre, string $apellido, string $telefono, string $password){
			$this->intIdUsuario = $idUsuario;
			...
			if($this->strPassword != "")
			{
				$sql = "UPDATE usuario SET  nombres=?, apellidop=?, telefono=?, password=? 
						WHERE idpersona = $this->intIdUsuario ";
				$arrData = array(
								$this->strNombre,
								$this->strApellido,
				...
				$sql = "UPDATE usuario SET nombres=?, apellidop=?, telefono=? 
						WHERE idpersona = $this->intIdUsuario ";
				$arrData = array(
								$this->strNombre,
								$this->strApellido,
