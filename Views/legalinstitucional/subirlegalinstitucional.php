...
$stmt = Conexion::conectar()->prepare("SELECT count(IdInstitucional) cant FROM baseintitucional where Habilitado = 1 order by FechaRegistro desc");
$stmt->execute();
$fila =$stmt->fetch();
...
if(!file_exists('../../Assets/archivos/basenacional')){
  ...
  if(file_exists('../../Assets/archivos/basenacional/')){
    if(move_uploaded_file($ubicacionTemporal,'../../Assets/archivos/basenacional/'.$nuevonombre)){
        ...
        $stmt = Conexion::conectar()->prepare("INSERT INTO baseintitucional(Nombre,NombreArchivo,Posicion)
