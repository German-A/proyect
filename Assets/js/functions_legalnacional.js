...
document.addEventListener('DOMContentLoaded', function() {
    ...
    datatable = $('#datatable').dataTable({
        ...
        // },
        "ajax": {
            "url": " " + base_url + "/legalnacional/get",
            "dataSrc": ""
        },
        "columns": [
            { "data": "IdNacional" },
    ...
    if (document.querySelector("#formmodal")) {
        ...
        formmodal.onsubmit = function(e) {
            ...
            let ajaxUrl = base_url + '/legalnacional/set';
            ...
            request.onreadystatechange = function() {
                if (request.readyState == 4 && request.status == 200) {
                    ...
                    if (objData.status) {
                        ...
                        formmodal.reset();
                        swal("Documento Legal", objData.msg, "success");
                    } else {
...
function fntView(idbtn) {
    ...
    let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    let ajaxUrl = base_url + '/legalnacional/getone/' + idbtn;
...
function fntEdit(element, idbtn) {
    ...
    document.querySelector('#titleModal').innerHTML = "Actualizar Imagen en el legalnacional";
    ...
    request.onreadystatechange = function() {
        ...
        if (request.readyState == 4 && request.status == 200) {
            ...
            if (objData.status) {
                document.querySelector("#id").value = objData.data.IdNacional;
...
function fntDelete(IdNacional) {
    ...
    //let idlegalnacional = idbtn;
    swal({
        title: "Eliminar legalnacional",
        text: "¿Realmente quiere eliminar el legalnacional?",
        type: "warning",
        ...
        if (isConfirm) {
            let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
            let ajaxUrl = base_url + '/legalnacional/delete/' + IdNacional;
